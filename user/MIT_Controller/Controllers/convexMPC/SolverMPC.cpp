...
static unsigned int someCountMPC = 0;
const int printIndexMPC = 100;
static nlohmann::json dataJsonMPC;
...
Eigen::Matrix3f skewMat(Eigen::Vector3f input)
{
    Eigen::Matrix3f skewMat;
    skewMat << 0.0, -input(2), input(1), input(2), 0.0, -input(0), -input(1), input(0), 0.0;
    return skewMat;
}

using ABdt_t = std::pair<Eigen::Matrix<float, 13, 13>, Eigen::Matrix<float, 13, 12>>;
ABdt_t generateAdt_Bdt(const Eigen::Quaternionf &baseRotation, const double dtMPC,
                       const std::array<Eigen::Vector3f, 4> &footDistFromCOM)
{

    Eigen::Matrix<float, 13, 13> A_dt;
    Eigen::Matrix<float, 13, 12> B_dt;
    ...
    Eigen::Matrix3f I_world = rotMat * I_body * rotMat.transpose();
    Eigen::Matrix3f I_world_inv;
    ...
    A_dt.block<3, 3>(3, 9) = Eigen::Matrix3f::Identity() * dtMPC;
    A_dt(11, 12) = dtMPC;

    for (int i = 0; i < 4; i++)
    {
        B_dt.block(6, i * 3, 3, 3) = I_world_inv * skewMat(footDistFromCOM[i]);
        B_dt.block(9, i * 3, 3, 3) = Eigen::Matrix3f::Identity() / mass;
    }
...
void c2qp(Matrix<fpt, 13, 13> a, Matrix<fpt, 13, 12> b, fpt dt, s16 horizon)
{
    //  ABc.setZero();
    ...
    //  Adt(5,12) = 0.0;
    //  Bdt.block<6,12>(0,0) = Eigen::Matrix<fpt, 6,12>::Zero();
    ...
    {
        nlohmann::json aqp, bqp, adt, bdt;
        Eigen::to_json(aqp, A_qp);
        Eigen::to_json(bqp, B_qp);
        Eigen::to_json(adt, Adt);
        Eigen::to_json(bdt, Bdt);
        dataJsonMPC["A_qp"] = aqp;
        dataJsonMPC["B_qp"] = bqp;
        dataJsonMPC["A_dt"] = adt;
        dataJsonMPC["B_dt"] = bdt;
    }
#ifdef K_PRINT_EVERYTHING
    cout << "AQP:\n" << A_qp << "\nBQP:\n" << B_qp << endl;
#endif
...
{
    someCountMPC++;
    std::cout << "someCountMPC: " << someCountMPC << std::endl;
    ...
    if (someCountMPC == printIndexMPC)
    {
        nlohmann::json quat, footDistFromCoM;
        Eigen::to_json(quat, rs.q);
        Eigen::to_json(footDistFromCoM, rs.r_feet.transpose());
        dataJsonMPC["baseRotation"] = quat;
        dataJsonMPC["footDistFromCoM"] = footDistFromCoM;
        dataJsonMPC["dtMPC"] = setup->dt;
        dataJsonMPC["horizon"] = setup->horizon;
    }
    quat_to_rpy(rs.q, rpy);

    Eigen::Vector3f baseRotationEuler = rs.q.toRotationMatrix().eulerAngles(0, 1, 2);
    Eigen::Matrix3f R_yaw;
    double yc = cos(baseRotationEuler[2]);
    double ys = sin(baseRotationEuler[2]);
    R_yaw << yc, -ys, 0, ys, yc, 0, 0, 0, 1;

    //    std::cout << "Quat Rot Mat:\n" << rs.q.toRotationMatrix() << std::endl;
    //    std::cout << "Rpy from quat:\n" << baseRotationEuler << std::endl;
    //    std::cout << "Quat Yaw Rot Mat:\n" << R_yaw << std::endl;
    //    std::cout << "R_yaw:\n" << rs.R_yaw << std::endl;
    //    std::cout << "R:\n" << rs.R << std::endl;
    //    std::cout << "yaw:\n" << rs.yaw << std::endl;
    ...
    std::array<Eigen::Vector3f, 4> footDistFromCoMArray{};
    for (unsigned i = 0; i < 4; i++)
    {
        footDistFromCoMArray[i] = (rs.r_feet.block<3, 1>(0, i)).transpose();
    }
    auto Adt_bdt_pair = generateAdt_Bdt(rs.q, setup->dt, footDistFromCoMArray);
    Adt = Adt_bdt_pair.first;
    Bdt = Adt_bdt_pair.second;
    c2qp(Eigen::Matrix<float, 13, 13>::Zero(), Eigen::Matrix<float, 13, 12>::Zero(), setup->dt, setup->horizon);
    ...
    if (someCountMPC == printIndexMPC)
    {
        nlohmann::json q_sol_json, q_sol_reduced_json, H_red_json, g_red_json, A_red_json, lb_red_json, ub_red_json;
        Eigen::Map<Eigen::Matrix<double, 120, 1>> q_sol_map(q_soln, 120);
        Eigen::Map<Eigen::Matrix<double, 60, 1>> q_sol_reduced_map(q_red, 60);
        Eigen::Map<Eigen::Matrix<double, -1, -1, Eigen::RowMajor>> H_red_map(H_red, new_vars, new_vars);
        Eigen::Map<Eigen::Matrix<double, -1, -1, Eigen::RowMajor>> g_red_map(g_red, new_vars, 1);
        Eigen::Map<Eigen::Matrix<double, -1, -1, Eigen::RowMajor>> A_red_map(A_red, new_cons, new_vars);
        Eigen::Map<Eigen::MatrixXd> lb_red_map(lb_red, new_cons, 1);
        Eigen::Map<Eigen::MatrixXd> ub_red_map(ub_red, new_cons, 1);
        Eigen::to_json(q_sol_json, q_sol_map);
        Eigen::to_json(q_sol_reduced_json, q_sol_reduced_map);
        Eigen::to_json(H_red_json, H_red_map);
        Eigen::to_json(g_red_json, g_red_map);
        Eigen::to_json(A_red_json, A_red_map);
        Eigen::to_json(lb_red_json, lb_red_map);
        Eigen::to_json(ub_red_json, ub_red_map);
        dataJsonMPC["q_sol"] = q_sol_json;
        dataJsonMPC["q_sol_reduced"] = q_sol_reduced_json;
        dataJsonMPC["H_reduced"] = H_red_json;
        dataJsonMPC["g_reduced"] = g_red_json;
        dataJsonMPC["constraints_reduced"] = A_red_json;
        dataJsonMPC["constraints_lb_reduced"] = lb_red_json;
        dataJsonMPC["constraints_ub_reduced"] = ub_red_json;
        std::ofstream o("/home/pohzhiee/Documents/data_mpc.json");
        o << std::setw(4) << dataJsonMPC << std::endl;
    }
